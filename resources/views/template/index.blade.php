        ...
        <div id="wrapper">
            ...
            <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
                ...
                <a class="sidebar-brand d-flex align-items-center justify-content-center" href="{{ url('dashboard') }}">
                    ...
                    <div class="sidebar-brand-text mx-3">{{ str_replace('_', ' ', Auth::user()->status_pengguna) }}
                    </div>
                </a>
                    ...
                    <div id="collapseUtilities" class="collapse" aria-labelledby="headingUtilities"
                        ...
                        <div class="bg-white py-2 collapse-inner rounded">
                            ...
                            <a class="collapse-item" href="{{ url('pegawai') }}"
                                @if (Auth::user()->status_pengguna != 'SUPER_ADMIN') hidden @endif>Employee</a>
                        </div>
                    </div>
                </li>
                {{-- Transaction --}}
                <li class="nav-item " @if (Auth::user()->status_pengguna != 'ACCOUNTING_ADMIN' &&
                    ...
                    <div id="Transaction" class="collapse" aria-labelledby="headingUtilities"
                        ...
                        <div class="bg-white py-2 collapse-inner rounded">
                            ...
                            <a class="collapse-item" href="{{ url('quotation') }}"
                                @if (Auth::user()->status_pengguna == 'ACCOUNTING_ADMIN') {{ 'hidden' }} @endif>Quotation</a>
                            <a class="collapse-item" href="{{ url('sales') }}"
                                @if (Auth::user()->status_pengguna == 'ACCOUNTING_ADMIN') {{ 'hidden' }} @endif>Sales Order</a>
                            <a class="collapse-item" href="{{ url('purchase') }}"
                                @if (Auth::user()->status_pengguna == 'ACCOUNTING_ADMIN') {{ 'hidden' }} @endif>Purchase Order</a>
                            <a class="collapse-item" href="{{ url('goods') }}"
                                @if (Auth::user()->status_pengguna == 'SALES_ADMIN') {{ 'hidden' }} @endif>Goods Receipt</a>
                            <a class="collapse-item" href="{{ url('delivery') }}"
                                @if (Auth::user()->status_pengguna == 'SALES_ADMIN') {{ 'hidden' }} @endif>Delivery</a>
                ...
                {{-- report --}}
                {{-- @if (Auth::user()->status_pengguna != 'OWNER' && Auth::user()->status_pengguna != 'SUPER_ADMIN' && Auth::user()->status_pengguna != 'ACCOUNTING_ADMIN') hidden @endif --}}
                <li class="nav-item">
                    ...
                    <div id="Report" class="collapse" aria-labelledby="headingUtilities"
                        ...
                        <div class="bg-white py-2 collapse-inner rounded">
                            ...
                            <a class="collapse-item" href="{{ url('aging') }}"
                                @if (Auth::user()->status_pengguna != 'OWNER' &&
                                        Auth::user()->status_pengguna != 'SUPER_ADMIN' &&
                                        Auth::user()->status_pengguna != 'ACCOUNTING_ADMIN') hidden @endif>Aging Schedule</a>
                            <a class="collapse-item" href="{{ url('quotationReport') }}"
                                @if (Auth::user()->status_pengguna != 'OWNER' &&
                                        Auth::user()->status_pengguna != 'SUPER_ADMIN' &&
                                        Auth::user()->status_pengguna != 'SALES_ADMIN') hidden @endif>Quotation VS PO
                                Report</a>
                            <a class="collapse-item"
                                href="{{ url('quotationReportDetail') }}"@if (Auth::user()->status_pengguna != 'OWNER' &&
                                        Auth::user()->status_pengguna != 'SUPER_ADMIN' &&
                                        Auth::user()->status_pengguna != 'SALES_ADMIN') hidden @endif>Quotation
                                Detail
                                Report</a>
                            <a class="collapse-item" href="{{ url('customerOmzetReport') }}">Customer Omzet
                                Report</a>
                            ...
                            <a class="collapse-item" href="{{ url('paymentvendor/report/report') }}"
                                @if (Auth::user()->status_pengguna != 'OWNER' && Auth::user()->status_pengguna != 'SUPER_ADMIN') hidden @endif>Payment To Vendor
                                Report</a>
                            <a class="collapse-item" href="{{ url('omzetReport') }}"
                                @if (Auth::user()->status_pengguna != 'OWNER' &&
                                        Auth::user()->status_pengguna != 'SUPER_ADMIN' &&
                                        Auth::user()->status_pengguna != 'ACCOUNTING_ADMIN' &&
                                        Auth::user()->status_pengguna != 'SALES_ADMIN') hidden @endif>Omzet Report</a>
                            <a class="collapse-item" href="{{ url('status_transaksi') }}"
                                @if (Auth::user()->status_pengguna != 'OWNER' &&
                                        Auth::user()->status_pengguna != 'SUPER_ADMIN' &&
                                        Auth::user()->status_pengguna != 'ACCOUNTING_ADMIN') hidden @endif>Transaction Status </a>
                            <a class="collapse-item" href="{{ url('transaction_number_tracking') }}"
                                @if (Auth::user()->status_pengguna != 'OWNER' &&
                                        Auth::user()->status_pengguna != 'SUPER_ADMIN' &&
                                        Auth::user()->status_pengguna != 'ACCOUNTING_ADMIN') hidden @endif>Transaction Tracking
                            </a>
                                            ...
                                            <a class="dropdown-item d-flex align-items-center"
                                                ...
                                                <div>
                                                    <div class="small text-gray-500">
                                                        ...
                                                        {{ date('Y') }}</div>
                                                    <span class="font-weight-bold">Announcement Notice: You have a
                                                        notification regarding recivable {{ $n->no_transaksi }} click
                                                        for
                                                        more</span>
                                                </div>
                                            </a>
