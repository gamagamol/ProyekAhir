...
<<<<<<< HEAD
    ...
    //             $nama_produk = str_replace(' ', '_', $d->nama_produk) . '-' . ($i + 1);
    //             $newWordFileName = "stiker_item_$nama_produk.docx";
    //             $template->saveAs(public_path('stiker/' . $newWordFileName));
    //             $zip->addFile(public_path('stiker/' . $newWordFileName), $newWordFileName);
    //             // Menggunakan $newWordFileName sebagai nama dalam ZIP
    ...
    //     foreach ($data as $i => $d) {
    //         $worksheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, str_replace(' ', '_', $d->nama_produk));
    //         $spreadsheet->addSheet($worksheet, $i);
    //         $spreadsheet->setActiveSheetIndex($i);

    //         $worksheet->getCell('D2')->setValue('nama_pelanggan', $d->nama_pelanggan);
    //         $worksheet->getCell('D3')->setValue('no_pengiriman', $d->no_pengiriman);
    //         $no_penjualan = ($d->no_po_customer == '-' || $d->no_po_customer == '') ? $d->no_penjualan : $d->no_po_customer;
    //         $worksheet->getCell('D4')->setValue('no_penjualan', $no_penjualan);
    //         $worksheet->getCell('D6')->setValue('nomor_pekerjaan', $d->nomor_pekerjaan);
    //         $worksheet->getCell('D7')->setValue('layanan', $d->layanan);
    //         $worksheet->getCell('D8')->setValue('nama_produk', $d->nama_produk);
    //         $material_size = "$d->tebal_transaksi x $d->lebar_transaksi x $d->panjang_transaksi";
    //         // $worksheet->getCell('D9')->setValue('material_size', $material_size);
    //         // $worksheet->getCell('D10')->setValue('jumlah_pengiriman', $d->jumlah_detail_pengiriman . ' ' . 'PCS');
    //         // $worksheet->getCell('D11')->setValue('berat_pengiriman', $d->berat_detail_pengiriman . ' ' . 'Kg');
    //     }
    //     $namaFile = $data[0]->no_pengiriman;
    //     $writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($spreadsheet, 'Xls');
    //     header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
...
=======
    public function printStiker($no_transaksi)
    {
        ...
        return response()->download("stiker/$zipFileName")->deleteFileAfterSend();
>>>>>>> 065ce2d0f7c521301941af2609fff6bc618cb5e5
    }
}
